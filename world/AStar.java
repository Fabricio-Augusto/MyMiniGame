...
public class AStar {
		...
		// metodo para verificar se o Node ja esta na lista
		...
		// findPath => achar/encontar caminho
		public static List<Node> findPath(World word, Vector2i start, Vector2i end){
			...
			while(openList.size() > 0) {
				...
				for(int i=0;i<9; i++) {
					...
					if(tile instanceof MineWall) continue;
					if(i == 0) {
						Tile teste = World.tiles[x + xi + 1 + ((y + yi) * World.WIDTH)];
						Tile teste2 = World.tiles[x + xi + ((y + yi + 1) * World.WIDTH)];
						if(teste instanceof WallTile || teste instanceof MineWall || teste2 instanceof WallTile 
								|| teste2 instanceof MineWall) {
							continue;
						}
					}
					else if(i == 2) {
